...
To use GLPK you need to install the 'cffi_glpk' python package (with pip)
"""
...
from glpk_cffi import ffi
from glpk_cffi.lib import glp_find_col, glp_get_col_prim, glp_get_col_dual, GLP_CV, GLP_IV, GLP_BV, GLP_UNDEF, \
    GLP_FEAS, GLP_INFEAS, GLP_NOFEAS, GLP_OPT, GLP_UNBND, glp_set_col_kind, glp_find_row, glp_get_row_prim, \
    glp_get_row_dual, glp_get_obj_val, glp_set_obj_dir, glp_init_smcp, glp_init_iocp, GLP_MIN, GLP_MAX, GLP_ON, \
    GLP_OFF, GLP_MSG_OFF, GLP_MSG_ERR, GLP_MSG_ON, GLP_MSG_ALL, glp_term_out, glp_create_index, glp_create_prob, \
    glp_get_num_rows, glp_get_num_cols, glp_get_col_name, glp_get_col_lb, glp_get_col_ub, glp_get_col_kind, \
    glp_set_prob_name, glp_read_prob, glp_set_obj_coef, glp_simplex, glp_intopt, glp_get_status, glp_add_cols, \
    glp_set_col_name, glp_del_cols, glp_add_rows, glp_set_row_name, glp_write_lp, glp_write_prob, glp_set_mat_row, \
    glp_set_col_bnds, glp_set_row_bnds, GLP_FR, GLP_UP, GLP_LO, GLP_FX, GLP_DB, glp_del_rows, glp_get_mat_row, \
    glp_get_row_ub, glp_get_row_type, glp_get_row_lb, glp_get_row_name, glp_get_obj_coef, glp_get_obj_dir, \
...
class Variable(interface.Variable):
    ...
    def _index(self):
        if self.problem is not None:
            i = glp_find_col(self.problem.problem, str(self.name).encode())
            if i != 0:
                return i
            else:
                raise IndexError(
    ...
    def name(self, value):
        if getattr(self, 'problem', None) is not None:
            glp_set_col_name(self.problem.problem, glp_find_col(self.problem.problem, self.name), str(value).encode())
...
class Constraint(interface.Constraint):
    ...
    def _get_expression(self):
        if self.problem is not None:
            ...
            ia = ffi.new("int[{}]".format(col_num +1))  # intArray(col_num + 1)
            da = ffi.new("double[{}]".format(col_num + 1))  # doubleArray(col_num + 1)
    ...
    def name(self, value):
        ...
        self._name = value
        if self.problem is not None:
            glp_set_row_name(self.problem.problem, glp_find_row(self.problem.problem, old_name), str(value).encode())
    ...
    def _index(self):
        try:
            i = glp_find_row(self.problem.problem, str(self.name).encode())
            if i != 0:
                return i
            else:
                raise IndexError(
...
class Model(interface.Model):
    ...
    def from_lp(cls, lp_form):
        ...
        with TemporaryFilename(suffix=".lp", content=lp_form) as tmp_file_name:
            glp_read_lp(problem, ffi.NULL, tmp_file_name.encode())
    ...
    def __setstate__(self, repr_dict):
        with TemporaryFilename(suffix=".glpk", content=repr_dict["glpk_repr"]) as tmp_file_name:
            problem = glp_create_prob()
            glp_read_prob(problem, 0, tmp_file_name.encode())
    ...
    def to_lp(self):
        ...
        with TemporaryFilename(suffix=".lp") as tmp_file_name:
            glp_write_lp(self.problem, ffi.NULL, tmp_file_name.encode())
    ...
    def _glpk_representation(self):
        ...
        with TemporaryFilename(suffix=".glpk") as tmp_file_name:
            glp_write_prob(self.problem, 0, tmp_file_name.encode())
            with open(tmp_file_name) as tmp_file:
    ...
    def _remove_variables(self, variables):
        if len(variables) > 0:
            ...
            num = ffi.new("int[{}]".format(len(variables) + 1))#intArray(len(variables) + 1)
    ...
    def _add_constraints(self, constraints, sloppy=False):
        ...
        for constraint in constraints:
            ...
            glp_set_row_name(self.problem, index, str(constraint.name).encode())
            num_cols = glp_get_num_cols(self.problem)
            index_array = ffi.new("int[{}]".format(num_cols + 1))#intArray(num_cols + 1)
            value_array = ffi.new("double[{}]".format(num_cols + 1))#doubleArray(num_cols + 1)
    ...
    def _remove_constraints(self, constraints):
        if len(constraints) > 0:
            ...
            num = ffi.new("int[{}]".format(len(constraints) + 1))#intArray(len(constraints) + 1)
...
if __name__ == '__main__':
    ...
    problem = glp_create_prob()
    glp_read_lp(problem, ffi.NULL, "../tests/data/model.lp".encode())
